...
dim = 10
sampler_BPS = BPSAD(dim, U, grid_size=10, refresh_rate = 0.0)
...
t_BPS_mixed, s_BPS_mixed = get_radial_momentum(sampler_BPS, x_BPS, v_BPS, t_BPS)

plot(t_BPS_mixed[1:15], s_BPS_mixed[1:15],
     xlabel=L"t",
     ylabel=L"⟨x|v⟩",
     label="Inner Product",
     linewidth=1,
...
anim_traj(output_BPS, 300; filename="BPS_$(dim)d_Trajectory.gif", title="Bouncy Particle Sampler, d=$(dim)")